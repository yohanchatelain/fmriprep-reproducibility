...
def _hash(args, ext):
    keys = dict(prefix=args.reference_prefix,
    ...
    _hash = '_'.join(map(lambda k: str(k).replace(
        os.path.sep, '-').replace('.', '_'), keys.values()))
    return _hash
...
def _get_reference(args, ext):
    ...
    if _has_memoization(args, ext):
        ...
        references, reference_masks = mri_image.get_reference(
            ...
            reference_ext=ext)
        references_masked, _ = mri_image.mask_t1(
            references, reference_masks, args.mask_combination, args.smooth_kernel)
...
def compute_distance(args):
    columns = ['class',
               'reference', 'dataset', 'subject', 'template', 'fwhm',
