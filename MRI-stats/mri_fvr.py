...
def compute_fvr(methods, target, confidences, *args, **info):
    ...
    def print_sep():
        return print(mri_printer.sep_h3) if len(methods) > 1 else lambda: None
...
def compute_pvalues_stats(
    args, ith_target, target_T1, supermask, parameters, weights, methods, **info
):
    ...
    mri_printer.print_info(score_name, sample_size, target_filename, ith_target)
    ...
    fvr = compute_fvr(methods, target_T1, confidences, p_values, **info)

    return fvr


def compute_sig_stats(
    args, ith_target, references_T1, target_T1, supermask, sig, methods, **info
):
    ...
    mri_printer.print_info("Sigbit", sample_size, target_filename, ith_target)

    test_sig = significantdigits.significant_digits(
        references_T1,
    ...
    fvr = compute_fvr(methods, target_T1, confidences, ref_sig, test_sig, **info)
...
def compute_fvr_per_target(
    args, references_T1, targets_T1, supermask, methods, nb_round=None, kth_round=None
):
    """
    ...
    if args.gmm:
        ...
        sig = significantdigits.significant_digits(
            references_T1, reference=mean, axis=0, error=sig_error, method=sig_method
        )
    elif args.gaussian_type == "skew":
        if args.parallel_fitting:
            parameters = parallel_fit_normal(references_T1, scipy.stats.skewnorm.fit)
        else:
            parameters = sequential_fit_normal(references_T1, scipy.stats.skewnorm.fit)

    elif args.gaussian_type == "general":
        if args.parallel_fitting:
            parameters = parallel_fit_normal(references_T1, scipy.stats.gennorm.fit)
        else:
            parameters = sequential_fit_normal(references_T1, scipy.stats.gennorm.fit)
...
def compute_k_fold_fvr(args, reference_t1, reference_mask, nb_rounds, methods):
    ...
    def compute_k_fold_round(i, train_id, test_id):
        ...
        train_t1_masked, supermask = mri_image.mask_t1(args, train_t1, train_mask)
    ...
    fvr_list = [
        ...
        for i, (train_id, test_id) in enumerate(kfold.split(reference_t1), start=1)
    ]
...
def compute_stats(args):
    ...
    t1s_masked, supermask = mri_image.mask_t1(args, reference_t1s, reference_masks)
