...
def print_header1(confidence, dataset, subject, method, mean, std):
    ...
    msg = (f'{bcolors.BOLD}{bcolors.WARNING}Confidence: {confidence}{bcolors.ENDC}, '
           f'Dataset: {dataset}, '
           f'Subject: {subject}, '
...
def print_status(confidence, dataset, subject, method, status, to_pass, mean, std):
    ...
    for row in raw_stats:
        ...
        _fvr = row['fvr']
        _target = row['target']
        if _confidence == confidence and\
                _dataset == dataset and\
                _subject == subject and\
                method == _method:
            if _status:
...
def compute_stats(status_check):
    ...
    for key, fvrs in fvr_stats.items():
        ...
        print_status(confidence, dataset, subject, method,
                     counters, status_check, fvr_mean, fvr_std)


def parse_args():
    parser = argparse.ArgumentParser(description='check status')
    parser.add_argument("--status", required=True, choices=status_choices.keys(),
                        help='Expected status')
    parser.add_argument('--filename', required=True,
                        help="Filename that contains statistics")
