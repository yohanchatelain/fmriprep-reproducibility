...
def get_figure(args):
    ...
    fig = make_subplots(rows=3, cols=3,
                    ...
                    row_titles=['Mean' + (' (log)' if args.mean_log else ''),
                                'Standard deviation' + (' (log)' if args.std_log else ''),
                                'Significant bits'],
                    x_title='FWHM')
...
def parse_args():
    ...
    parser.add_argument('--mean-log', action='store_true')
    parser.add_argument('--std-log', action='store_true')
...
def main():
    ...
    if args.output:
        fig.write_image(args.output, scale=10)
